...
using Shin.IoC.DependencyInjection.Registration;
...
{
    public sealed partial class ShinContainer
    {
        #region Methods
        ...
        public void Register<T, C>(bool asSingleton = true,
            ...
            where C : class, T
        {
            CreateResolver<T, C>(key, asSingleton, overrideExisting);
        }
                                     ...
                                     DIResolutionStrategy strategy = DIResolutionStrategy.Default)
        {
            ...
            {
                var typeId = ResolveTypeId(T);
                result = m_typeDictionary.ContainsKey(typeId);

                if (!result &&
                    m_interfaceCache.IsRegistered(T, out var cType) &&
                    cType is not null)
                    foreach (var t in cType)
                    {
                        var cTypeId = ResolveTypeId(t);
                        result = m_typeDictionary.ContainsKey(cTypeId) /*|| m_interfaceDictionary.ContainsKey(T)*/;
                    }
            }
        ...
        private ITypeResolver CreateResolver(ITypeRegistrationContext context)
        //Type T,
        //                        Type C,
        //                        string key = null,
        //                        bool asSingleton = false,
        ...
        private bool CheckResolver(Guid typeId,
                                    out TypeResolverDictionary resolverDictionary)
        {
            resolverDictionary = null;
            m_typeDictionaryLockSlim.TryEnter();
            try
            {
                if (m_typeDictionary.TryGetValue(typeId, out resolverDictionary))
                    return true;

                //if (m_typeDictionary.TryGetValue(cT, out resolverDictionary)) return true;
            }
            finally
            {
        ...
        private void CreateRegistrationCore(ref ITypeRegistrationContext context)
        {
            var reg = new TypeRegis
            m_registrationDictionary.TryAdd(context.Interface, );
        }

        private void CreateContext(Type T,
                                   Type cT,
                                   string key,
                                   bool asSingleton,
        ...
        private ITypeResolver CreateResolverCore(ITypeRegistrationContext context)
        {
            m_typeDictionaryLockSlim.TryEnter(SynchronizationAccess.Write);
            try
            {
                var T = context.Interface.ToType();
                var cT = context.Concrete.ToType();

                if ((cT.IsInterface || cT.IsAbstract))
                ...
                //var typeId = ResolveTypeId(T);
                //var cTypeId = ResolveTypeId(cT);
                //key ??= typeId;

                m_interfaceCache.Register(T);
                //RegisterInterfaces(T, cT);

                //key = GetKeyValueOrDefault(key) ?? key;
                //ResolverDictionary resolverDictionary = null;
                if (!CheckResolver(context.Concrete, out var resolverDictionary))
                    resolverDictionary ??= new TypeResolverDictionary();

                if (context.IsSingleton && resolverDictionary.TryGetValue(context.Concrete, out var r)) return;

                var resolver = context.InstanceMethod is not null
                                   ? new TypeResolver(this, cT, context.IsSingleton, context.InstanceMethod)
                                   : context.Instance is not null
                                       ? new TypeResolver(this, cT, context.IsSingleton, context.InstanceMethod)
                                       : new TypeResolver(this, cT, context.IsSingleton);
                resolver.Initialize();
                //resolverDictionary[typeId] = resolver;
                m_typeDictionary[context.Concrete] = resolver;
                m_keyDictionary[context.Key] = cTypeId;
            }
            finally
            {
