...
namespace Shin.IoC.DependencyInjection
{
    public sealed partial class ShinContainer
    {
        ...
        {
            return null; //ResolveTypeId(typeof(T));
        }

        private static Guid ResolveTypeId(Type type)
        {
        ...
        private IEnumerable<object> ResolveAllSelf(Type type, string key = null, params object[] parameters)
        {
            ...
            if (!m_interfaceCache.IsRegistered(type, out var cType))
            {
                ...
                try
                {
                    ...
                    if (m_typeDictionary.TryGetValue(typeId, out var resolvers))
            ...
            foreach (var t in cType)
            {
                ...
                try
                {
                    ...
                    if (!m_typeDictionary.TryGetValue(cTypeId, out var resolvers)) continue;
        ...
        private object ResolveSelf(Type type, string key = null, params object[] parameters)
        {
            ...
            m_interfaceCache.IsRegistered(type, out var cType);

            foreach (var t in cType)
            {
                //var cTypeId = ResolveTypeId(t);
                if (!CheckResolver(t, out var resolvers))
                    continue;
        ...
        internal ShinContainer ResolveContainer(string key)
        {
            return this;
        }
                                       ...
                                       IDictionary<string, object> resolvedObjects = null)
        {
            ...
            foreach (var propertyInfo in injectableProperties)
            {
                ...
                {
                    setter = ExpressionCompiler.CreateSetter(propertyInfo);
