...
namespace Shin.IoC.DependencyInjection
{
    internal static class ExpressionCompiler
    {
        ...
        public static Delegate CreateDelegate(PropertyInfo property)
        {
            return CreateCompiledExpression(property);
        }

        public static Delegate CreateDelegate(ConstructorInfo constructor, params ParameterInfo[] paramInfos)
        {
            var pInfos = paramInfos?.Length > 0 ? paramInfos : constructor.GetParameters();
            return CreateCompiledExpression(constructor, pInfos);
        }

        //public static Delegate CreateDelegate(ConstructorInfo constructor)
        //{
        ...
        public static Expression CreateExpression(ConstructorInfo method,
        ...
        static Delegate CreateCompiledExpression(PropertyInfo property)
        {
            var setMethod = property.CanWrite
                                ? property.GetSetMethod(true)
                                : throw new
                                      ArgumentException($"Property {property.DeclaringType}.{property.Name} has no setter or parameters Length not equal to 1.");

            var obj = Expression.Parameter(typeof(object), "o");
            var value = Expression.Parameter(typeof(object));

            var expr = Expression.Lambda(Expression.Call(Expression.Convert(obj, setMethod.DeclaringType),
                                                                                 setMethod,
        ...
        public static Func<object, object> CreateGetter(PropertyInfo property)
        {
            ...
            var method = typeof(ExpressionCompiler).GetMethod(nameof(CoerceCompiled), BindingFlags.Static | BindingFlags.NonPublic);
